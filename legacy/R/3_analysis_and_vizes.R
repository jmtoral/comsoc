...
rel <- clean_data %>%
  ...
  mutate(nombre = case_when(
    ...
    str_detect(nombre, "SEDATU") ~ "SEDATU",
    str_detect(nombre, "SGM") ~ "SGM",
    str_detect(nombre, "COMISIÓN EJECUTIVA DE ATENCIÓN A VICTIMAS|PROVICTIMA") ~ "CEAV",
    TRUE ~ nombre
  )) |> 
  mutate(yr = floor_date(fecha_de_gasto, "year")) %>%
  filter(yr >= as.Date('2012-01-01') & yr <= as.Date('2023-01-01') ) |> 
...
ggsave("graphs/red_amlo.png", width = 12, height = 12, dpi = 120, bg = "white")
...
ggsave("graphs/red_epn.png", width = 12, height = 12, dpi = 120, bg = "white")


#clean_data |> filter(str_detect(nombre, "PROVICTIMA")) |> select(nombre)
